...
if merge
[x_e,vx_e,vy_e,vz_e,WEIGHT_e]=merging2(xN_e,xG_e,x_e,vx_e,vy_e,vz_e,WEIGHT_e,DX,N_G,8);
end
N_e=length(WEIGHT_e);
xG_i=zeros(N_G,1);
xN_i=zeros(1, N_G);
if mod(t, N_SUB) == 0
    ...
    if merge
    [x_i,vx_i,vy_i,vz_i,WEIGHT_i]=merging2(xN_i,xG_i,x_i,vx_i,vy_i,vz_i,WEIGHT_i,DX,N_G,8);
    end
    N_i=length(WEIGHT_i);
end
if mod(t, 100) == 0
